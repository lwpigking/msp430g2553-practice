...
int counter=0;
/**
 * 8. 编程实现对P2.4的下降沿捕获，用LED亮灭观察捕获情况。
 * 9. 编程实现扩展板上的key1-key4键点动控制扩展板上的LED1-LED4灯，按下时灭，释放时亮。
 * 10. 利用I2C扩展IO单元的按键，编程实现4个键的按键显示功能。上电时清LCD，按下Key1-Key4键时可在第六个大字符显示位置按键数字，释放时清显示数字。
 * 11. 编程实现用P1.3键实现0-9的循环按键计数功能，按一次计数器加一，并在LCD上显示。
 */

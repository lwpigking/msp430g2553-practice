...
/******************************************************************************************************
 * 名       称：TCA6416_Rx_Frame1()
 * 功       能：从TCA6416A接收一字节函数
 * 入口参数： *p_Rx：待存放数据的头指针
  * 出口参数：无
 * 说       明：调用底层驱动函数实现。收成功为止。
 * 范       例：无
 ******************************************************************************************************/
void TCA6416_Rx_Frame1(unsigned char *p_Rx)
{
	unsigned char temp=0;
	do {
		temp=I2C_RxFrame1(p_Rx);
	}while(temp==0);
}
/******************************************************************************************************
 * 名       称：TCA6416A_Init()
...
void PinIN()
{
	...
	TCA6416_Rx_Frame1(temp);								        // 读取IO输入寄存器
	//----将最新键值，更新到输入缓存----
   	TCA6416A_InputBuffer =(((unsigned int)temp[0])<<8 );
	I2C_Tx_Init();
}
